...
# DESKTOP_PATH = "C:\\Users\\linko\\OneDrive\\Desktop\\Model"
# CAFFE_MODEL = os.path.join(DESKTOP_PATH, "hed_pretrained_bsds.caffemodel")
# PROTOTXT = os.path.join(DESKTOP_PATH, "deploy.prototxt")
# OUTPUT_TFLITE = os.path.join(DESKTOP_PATH, "hed.tflite")
...
#     # Simplified HED architecture (adjust based on deploy.prototxt)
#     tf_model = tf.keras.Sequential([
#         tf.keras.layers.InputLayer(input_shape=(None, None, 3)),
#         tf.keras.layers.Lambda(lambda x: (x - [104.00698793, 116.66876762, 122.67891434]) / 255.0),
#         tf.keras.layers.ZeroPadding2D((1, 1)),
#         tf.keras.layers.Conv2D(64, (3, 3), activation='relu', name='conv1_1'),
#         # Add all layers from deploy.prototxt...
#     ])
    
#     # Transfer weights (example for conv1_1)
#     layer_name = 'conv1_1'
#     blobs = net.getLayer(layer_name).blobs
#     weights = np.array(blobs[0].data).transpose((2, 3, 1, 0))
#     biases = np.array(blobs[1].data)
#     tf_model.get_layer(layer_name).set_weights([weights, biases])
